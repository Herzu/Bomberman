    ...
    void SummonCharacters()
    {
        ...
        foreach(int type in controlls)
        {
            ...
            switch (type)
            {
                ...
                case 4:     //bot
                    //umieszczenie bota
                    GameObject bot = Instantiate(BotPrefabs[characters], pos[characters], Quaternion.identity);
                    BomberAgent agent = bot.GetComponent<BomberAgent>();
                    if (agent)
                    {
                        agent.gameController = this;
                        agent.target = GameObject.FindGameObjectWithTag("Player");
                    }
                    break;
            }
